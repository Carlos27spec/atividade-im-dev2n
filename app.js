...
function telaPerfil() {
  const usuarioLogado = usuarios["whats-users"][userAT.usuarioAtual];

  const listaPerfis = usuarios["whats-users"]
    .map(
      (usuario, index) => `
        <div class="perfil-item" data-index="${index}">
          <img src="https://i.pravatar.cc/150?u=${usuario.number}" alt="" />
  ...
  return `<h1 class="user">Perfil</h1>
    <div class="foto">
      <img src="https://i.pravatar.cc/150?u=${usuarioLogado.number}" alt="" />
    </div>
    <div class="info">
      <h2>Nome</h2>
      <p>${usuarioLogado.account}</p>
      <h2>Recado</h2>
      <div class="sit">
        <p>disponível</p>
...
Perfil.addEventListener("click", function () {
  ...
  if (telaAT) {
    ...
    container.innerHTML = telaPerfil();
  }
});
